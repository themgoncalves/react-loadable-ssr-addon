...
import getBundles from 'react-loadable-ssr-addon/getBundles';
...
// now we get the required files
// together with the application entry points files
let bundles = getBundles(manifest, [...manifest.entrypoints, ...modules]);

// `getBundles` provide us all files group by `file type`
